local yinx_radarStations1 = {}

local function yinx_getStorage1()
    return Util.optStorage(City.getStorage(), "yinx.radar.horizontal.data")
end
...
local function yinx_initStation(x, y)
    local id = x .. "_" .. y
    if not yinx_radarStations1[id] then
        local storage = yinx_getStorage1()
        local savedoffsetY = storage[id] or 1
        
        yinx_radarStations1[id] = { 
            x = x, y = y, 
...
function script:click(x, y, level)
    ...
    local station = yinx_radarStations1[id]
    
    if station then
        local nextoffsetY = station.offsetY + 1
        if nextoffsetY > 4 then
            nextoffsetY = 1
        end
        ...
        dialog = GUI.createDialog{
            ...
            text = yinx_getText("desc", station.offsetY, nextoffsetY),
            actions = {
                {
                ...
                },
                {
                    ...
                    onClick = function()
                        station.offsetY = nextoffsetY
                        
                        local storage = yinx_getStorage1()
                        storage[id] = station.offsetY
                        
                        Debug.toast(yinx_getText("toast_success", station.offsetY))
                        if dialog then dialog:close() end
                    end
                }
            }
...
function script:update()
    ...
    for id, station in pairs(yinx_radarStations1) do
        
        local roadY = station.y + station.offsetY
        
        local countUp = Tile.getRoadCarCount(station.x - 3, roadY, 2) or 0
        local countDown = Tile.getRoadCarCount(station.x + 3, roadY, 2) or 0
